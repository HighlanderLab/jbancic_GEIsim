...
save_output <- function(df, pop, stage, year, nenvs = NULL, gv_tpe, pheno_met = NULL) {
  ...
  # Main phenotypic effects when available
  ...
  df[index, c("mean_gv_met", "mean_gv_tpe")] <- c(mean(mean_gv_met), mean(mean_gv_tpe))
  df[index, c("var_gv_met", "var_gv_tpe")]   <- c(var(mean_gv_met),  var(mean_gv_tpe))
  df[index, c("acc_met", "acc_tpe", "met_tpe")] <- 
    c(if (is.null(pheno_met)) NA else cor(mean_gv_tpe, mean_pheno_met), 
      if (is.null(pheno_met)) NA else cor(mean_gv_met, mean_pheno_met), 
